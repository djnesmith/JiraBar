...
private struct QuerySection: View {
    ...
    var body: some View {
        ...
        TextField("Recently Closed JQL:", text: $recentlyClosedJQL)
            .textFieldStyle(RoundedBorderTextFieldStyle())
        Text("Adds a Recently Closed entry under TODO listing finished tickets, each showing its PRs including merged and closed ones. Full submenu, same as the main list — Jira usually still offers Reopen on a Done ticket, and comments and attachments often arrive after the fact. Empty switches it off. Order by statusCategoryChangedDate rather than resolutiondate, which many workflows never populate. Add \"AND project in (ABC, DEF)\" to keep other projects' idea of \"Done\" out.")
            .font(.footnote)
        ...
        TextField("Recently Seen JQL:", text: $recentlySeenJQL)
            .textFieldStyle(RoundedBorderTextFieldStyle())
        Text("Adds a Recently Seen entry listing tickets whose status you changed that are assigned to someone else and are not Done — the ones that leave your board the moment you hand them on, so nothing otherwise records that you touched them. Full submenu, same as the main list — these are still open and still moving, and the transitions offered are whatever Jira allows for that ticket right now. Empty switches it off. Ordered by updated rather than statusCategoryChangedDate, because a hand-off is often between two statuses in the same category and that field would not move. Add \"AND project in (ABC, DEF)\" to scope it.")
            .font(.footnote)
        ...
        TextField("Dashboard URL:", text: $dashboardURL)
            .textFieldStyle(RoundedBorderTextFieldStyle())
        Text("Optional. Adds an Open Dashboard entry to the menu. Accepts a full URL or a path that's appended to your Jira base. A board URL (…/boards/12?quickFilter=34) also drives the TODO list: its To Do column, those quick filters, in board rank order.")
            .font(.footnote)
