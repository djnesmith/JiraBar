...
- **Issue type colour** — `Bug` renders red, and `Epic` and any `… Initiative` type render purple, because those are the two distinctions worth spotting without reading. Every other type — `Task`, `Story`, `Improvement`, `Sub-task` and anything your instance defines — stays in the row's metadata grey on purpose: the row already carries the key, assignee and status colours, and colouring the majority type would spend the last of its contrast on the one value that carries no decision.
