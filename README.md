...
- **Instant refresh from scripts** — `notifyutil -p com.dnesmith.jirabar.refresh` from any shell refreshes the menu immediately, so automation that moves tickets over REST doesn't wait for the poll timer. A burst of notifications is debounced into a single refresh.
...
- **Add Comment / Add Flag / Upload Files** — the upload dialog supports drag-and-drop. The flag entry only appears when a Flagged custom-field id is configured; on a ticket that is already flagged it reads **Remove Flag** and clears the flag without a dialog, and the ticket's row carries a red flag icon.
...
Each ticket's submenu is the full one, the same as in the main list — transitions, the copy shortcuts, comment, flag, upload and your user-field shortcuts, plus the PR rows. Done is not the end of a ticket's life: Jira typically still offers Reopen on these, and a closed ticket takes a comment or an attachment perfectly well, since evidence tends to arrive after the fact. Whatever Jira allows for a ticket in its current status is what you get, because the transitions are fetched per ticket. Submenus are built the first time you open the section, so it costs nothing until then, and **Recently Closed Max Results** caps the list separately.
...
Rows show the key, the type and the **status** — the status is the whole point, since it is the thing you changed. Each ticket's submenu is the full one, same as the main list: these tickets belong to someone else now, but they are still open and still moving, and this is often the only place they still appear. The transitions are fetched per ticket, so you are offered exactly what Jira allows for that ticket in its current status, and Jira enforces its own permissions on the rest. One thing it does not enforce is the GitHub mirror: using a user-field shortcut with **Also update GitHub PR** left on will assign *you* to a PR on someone else's ticket and rewrite that PR's requested reviewers, dropping anyone who is in your mapping file but not on the ticket. Untick it there if that is not what you want.
